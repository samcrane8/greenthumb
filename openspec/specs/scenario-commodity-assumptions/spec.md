# scenario-commodity-assumptions Specification

## Purpose

Let a scenario carry its own commodity price assumptions per commodity-priced driver, so
each scenario computes its own price path while unset scenarios inherit the base — with
base edits moving the model baseline, alternate edits scoped to their scenario,
regeneration on timeline changes, manual-override unbinding, registry validation, and
flow through the core operation layer and both the API and MCP adapters as validate-on-
write operations, surfaced by a scenario-scoped commodity panel in the web app.
