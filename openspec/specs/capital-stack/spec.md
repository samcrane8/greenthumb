# capital-stack Specification

## Purpose

Let a model describe its financing structure as a ranked capital stack — asset
references plus an ordered set of tranches (senior debt, subordinated debt,
convertibles, preferred, common) — and derive a seniority waterfall and related
coverage, blended-cost, leverage, and dilution analysis from it. The stack stores
only definitions and references (never computed numbers), validates its references
against known series, flows through the core operation layer and both adapters, stays
integrity-safe under renames and removals, and ships as a default in the treasury
template.
