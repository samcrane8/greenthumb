# model-charts Specification

## Purpose

Let a model carry an ordered collection of persisted `Chart` definitions — each a
titled, kinded set of series referencing items or drivers by name — validated
against the model so no chart can point at a nonexistent series, with chart data
derived on demand per scenario (never cached) and create/update/remove exposed as
core validate-on-write operations surfaced identically through the API and MCP.
