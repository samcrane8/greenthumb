# actuals-replay Specification

## Purpose

Let a chosen item be driven by real observed history instead of the engine
formula, by replacing its definition with an actuals-backed input series — while
preserving the prior definition so the swap is reversible — as validate-on-write
operations in the core operation layer that flow through both the API and the MCP
adapters, with the API seeding replay values from the model's stored actuals.
