...
Give a model an optional editable `Dashboard` of ordered, grid-laid-out `Widget`s
(chart, stat, statement, or note) whose references are validated so no widget can
render broken, with add/update/remove/reorder implemented as core validate-on-write
operations surfaced through the API and MCP, and the web app rendering and editing
the dashboard while falling back to the statement view when a model has none.
