...
Score a model's forecast against its stored actuals: a basic backtest over the
actuals window, an out-of-sample holdout split that judges quality on untouched
data, and walk-forward evaluation that rolls the point-in-time (as-of) compute
across a sequence of cutoffs — all read-only and honoring the look-ahead-bias
guard, exposed through both the API and MCP adapters.
