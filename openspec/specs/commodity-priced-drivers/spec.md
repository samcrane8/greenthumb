# commodity-priced-drivers Specification

## Purpose

Let a driver bind to a commodity price model so its value series is generated from that
model over the current timeline, regenerated automatically when the timeline changes, and
validated against the commodity registry — flowing through the core operation layer and
both the API and MCP adapters as validate-on-write operations.
