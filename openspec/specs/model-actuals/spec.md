# model-actuals Specification

## Purpose

Store observed historical values as first-class data alongside the model (not in
the diffable model JSON), import them from CSV with a column-to-item mapping,
join forecast against actuals for a target item, and support point-in-time
(as-of) re-forecasting that freezes known history and guards against look-ahead
bias.
